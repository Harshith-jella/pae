...
DROP POLICY IF EXISTS "Allow user creation" ON user_profiles;
DROP POLICY IF EXISTS "Anyone can view active parking spaces" ON parking_spaces;
DROP POLICY IF EXISTS "Owners can view their own spaces" ON parking_spaces;
DROP POLICY IF EXISTS "Owners can insert their own spaces" ON parking_spaces;
DROP POLICY IF EXISTS "Owners can update their own spaces" ON parking_spaces;
DROP POLICY IF EXISTS "Users can view their own bookings" ON bookings;
DROP POLICY IF EXISTS "Users can create bookings" ON bookings;
DROP POLICY IF EXISTS "Space owners can view bookings for their spaces" ON bookings;
DROP POLICY IF EXISTS "Space owners can update booking status" ON bookings;
...
BEGIN
  ...
  -- Insert owner user
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'owner@pae.com') THEN
    INSERT INTO auth.users (
      instance_id, id, aud, role, email, encrypted_password, email_confirmed_at,
      raw_app_meta_data, raw_user_meta_data, created_at, updated_at
    ) VALUES (
  ...
  -- Insert regular user
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'user@pae.com') THEN
    INSERT INTO auth.users (
      instance_id, id, aud, role, email, encrypted_password, email_confirmed_at,
      raw_app_meta_data, raw_user_meta_data, created_at, updated_at
    ) VALUES (
...
-- Insert demo user profiles (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM user_profiles WHERE id = '11111111-1111-1111-1111-111111111111') THEN
    INSERT INTO user_profiles (id, email, name, role, is_verified, created_at, updated_at) 
    VALUES ('11111111-1111-1111-1111-111111111111', 'admin@pae.com', 'Admin User', 'admin', true, NOW(), NOW());
  END IF;

  IF NOT EXISTS (SELECT 1 FROM user_profiles WHERE id = '22222222-2222-2222-2222-222222222222') THEN
    INSERT INTO user_profiles (id, email, name, role, is_verified, created_at, updated_at) 
    VALUES ('22222222-2222-2222-2222-222222222222', 'owner@pae.com', 'Space Owner', 'owner', true, NOW(), NOW());
  END IF;

  IF NOT EXISTS (SELECT 1 FROM user_profiles WHERE id = '33333333-3333-3333-3333-333333333333') THEN
    INSERT INTO user_profiles (id, email, name, role, is_verified, created_at, updated_at) 
    VALUES ('33333333-3333-3333-3333-333333333333', 'user@pae.com', 'Regular User', 'user', true, NOW(), NOW());
  END IF;
END $$;

-- Insert demo parking spaces (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM parking_spaces WHERE id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM parking_spaces WHERE id = 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb') THEN
    INSERT INTO parking_spaces (
      id, owner_id, title, description, address, city, state, zip_code,
      latitude, longitude, price_per_hour, size, type, amenities, images,
      is_active, rating, review_count
  ...
  IF NOT EXISTS (SELECT 1 FROM parking_spaces WHERE id = 'cccccccc-cccc-cccc-cccc-cccccccccccc') THEN
    INSERT INTO parking_spaces (
      id, owner_id, title, description, address, city, state, zip_code,
      latitude, longitude, price_per_hour, size, type, amenities, images,
      is_active, rating, review_count
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM bookings WHERE id = 'dddddddd-dddd-dddd-dddd-dddddddddddd') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM bookings WHERE id = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee') THEN
    INSERT INTO bookings (
      id, user_id, space_id, start_date, end_date, start_time, end_time,
      total_hours, hourly_rate, total_amount, platform_fee, owner_earnings,
      status, payment_status
